...
try { //Start everything
    ...
    if (save !== null) {
        global.lastSave = handleOfflineTime();
        Notify(`欢迎回来，已离线 ${format(global.lastSave, { type: 'time', padding: false })}${oldVersion !== player.version ? `\n游戏已从 ${oldVersion} 更新至 ${player.version}` : ''}${globalSave.developerMode ?
            `\n加载用时 ${format((Date.now() - playerStart.time.started) / 1000, { type: 'time', padding: false })}` : ''}
        `);
        void simulateOffline(global.lastSave);
    } else {
        pauseGame(false);
